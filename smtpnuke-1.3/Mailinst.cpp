...
Mailinst::Mailinst(Database& db) : Table(db)
{
	m_new = true;
	m_host_spam = false;
	m_mail_spam = false;
...
Mailinst::Mailinst(Database& db,long id) : Table(db)
{
	std::string sql;
	sql = "select * from mailinst where id=" + Utility::l2string(id);
	Spawn(sql);
}


Mailinst::Mailinst(Database& db,const std::string& sql) : Table(db)
{
	Spawn(sql);
}


Mailinst::Mailinst(Database& db,const std::string& host,
...
/*
		q.execute("create table mailinst ("
			   ...
			   "subject_spam string"
*/
void Mailinst::Spawn(const std::string& sql)
{
	Query q(GetDatabase());
	m_new = true;
	m_host_spam = false;
...
printf("SQL: '%s'\n",sql.c_str());
	...
	else
	{
		...
		sql = sql + "where id=" + Utility::l2string(m_id);
printf("SQL: '%s'\n",sql.c_str());
		q.execute(sql);
	}
}
